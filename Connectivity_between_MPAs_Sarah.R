...
All_MPAs@data$FID_NSB_MP <- All_MPAs@data$NSB_MPA_ID #This is important step! Hacky way since the file structure changed to keep unique identifier as the NSB_MPA_ID field
...
write.csv(Con_table, paste0("./Connectivity_between_MPA_Sarah/output_keep/Con_table/Con_table", size_reduction_threshold, "year", year[year_time], "_pld", pld[pld_time], ".csv"))
...
size_reduction_directory <- paste0(pld_directory, "/size_reduction_threshold", size_reduction_threshold)
dir.create(paste0(size_reduction_directory))

##write out Con_df
write.csv(Con_df, paste0(size_reduction_directory, "/Con_df", size_reduction_threshold,"year", year[year_time], "_pld", pld[pld_time], ".csv"), row.names = F)
...
for (i in 1:length(pld)){
  ...
  filenames <- list.files(path= paste0("./Connectivity_between_MPA_Sarah/output_keep/Con_df/pld", pld[i], "/size_reduction_threshold", size_reduction_threshold), pattern= ".csv", full.names=TRUE, recursive=T)
  ...
  write.csv(Nonzero_Con_df, paste0(size_reduction_results_directory, "/NonzeroCon_df", size_reduction_threshold, "_mean_pld", pld[i], ".csv"))
