...
df <- matrix(NA, nrow = length(Marxan_Present), ncol = 1)
df <- as.data.frame(df)

for (i in 1:length(Marxan_Present)){
  
  File_1 <- read.table(Marxan_Present[i])
  ...
  File_2 <- read.table(Marxan_Future[i])
...
plds_raw <- gsub(pattern = "pld", replacement = "", Depth_class_Present_PLDs)  

colnames(df) <- "Percent similarity"
rownames(df) <- paste0("Current to Future PLD ", plds_raw)

write.csv(df, paste0("./Marxan_future/Sensitivity/Present_Future/", Depth_class, "/", Top_percent, "Sensitivity_vector.csv"))
