...
for (pld_time in 1:length(pld)){
  ...
  for (year_time in 1:length(year)){
    ...
    #Acquiring files
    filenames <- list.files(path=paste0("./cuke_future/ConData/G",year[year_time]), pattern=glob2rx(paste0("*para    1",formatC(pld[pld_time]+1, width = 3, format = "d", flag = "0"),"*")), full.names=TRUE,recursive=T)
